...
const Team = () => {
  ...
  const [inactiveMaintainersData, setInactiveMaintainersData] = useState(null)

  useEffect(() => {
    const fetchFile = async (file, setData) => {
      const url = `https://raw.githubusercontent.com/Evolution-X/www_gitres/main/team/${file}.json`
      try {
        ...
        console.error(`${file}.json fetch error:`, err)
      }
    }

    const fetchData = async () => {
      await Promise.all([
        fetchFile("team", setTeamData),
        fetchFile("maintainers", setMaintainersData),
        fetchFile("inactive_maintainers", setInactiveMaintainersData)
      ])
    }
  ...
  if (!teamData || !maintainersData || !inactiveMaintainersData) {
    ...
    >
      ...
      >
        <div className="-my-14 flex flex-col items-center justify-between text-xs">
          ...
          <Card
            list={maintainersData.maintainers}
            shadowOn={false}
            isMaintainer={true}
          />
          <div className="mt-12 hidden w-fit bg-[#040214] px-3 text-2xl xl:block">
      ...
      >
        <div className="-my-14 flex flex-col items-center justify-between text-xs">
          ...
          <Card
            list={inactiveMaintainersData.maintainers}
            shadowOn={false}
            isMaintainer={true}
          />
          <div className="mt-12 hidden w-fit bg-[#040214] px-3 text-2xl xl:block">
  ...
  return (
    <div className="z-10 grid gap-16 md:grid-cols-2 lg:grid-cols-3">
      {list.map((item, index) => {
        ...
        const devicesList =
          isMaintainer && item.devices ? (
            <div className="text-xs">
              {(expandedItems.has(index)
                ? item.devices
                : item.devices.slice(0, 5)
        ...
        const moreDevicesCount =
          isMaintainer && item.devices && item.devices.length > 5 ? (
            <button
